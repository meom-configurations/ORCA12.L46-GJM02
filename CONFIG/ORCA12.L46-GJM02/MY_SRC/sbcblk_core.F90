MODULE sbcblk_core
   ...
   !!----------------------------------------------------------------------
   !!  IN ORCA025.L75-MJM104 : change radiative fluxes at high latitude as done in
   !!     ORCA025.L75-MJM91b ( GiG corrections)

   !!----------------------------------------------------------------------
   !!   sbc_kata_alloc: allocate katamask arrays
   !!   sbc_blk_alloc : allocate qsr_modified and qlw_modified
...
! most of the PARAMETERS are made PUBLIC because they are used somehow in the dia_wri routine (flx)      
   ...
   INTEGER , PUBLIC, PARAMETER ::   jp_wu   = 1   ! index of WIND * U10 (m2/s2) at T-point
   INTEGER , PUBLIC, PARAMETER ::   jp_wv   = 2   ! index of WIND * V10 (m2/s2) at T-point
   INTEGER , PUBLIC, PARAMETER ::   jp_wnd  = 3   ! index of 10m wind velocity module (m/s)    at T-point
   ...
   INTEGER , PUBLIC, PARAMETER ::   jp_tair = 7   ! index of 10m air temperature             (Kelvin)
   INTEGER , PUBLIC, PARAMETER ::   jp_prec = 8   ! index of total precipitation (rain+snow) (Kg/m2/s)
   INTEGER , PUBLIC, PARAMETER ::   jp_ccov = 9   ! index of total precipitation (rain+snow) (Kg/m2/s)
   INTEGER , PUBLIC, PARAMETER ::   jp_snow = 10  ! index of snow (solid prcipitation)       (kg/m2/s)
   INTEGER , PUBLIC, PARAMETER ::   jp_tdif = 11  ! index of tau diff associated to HF tau   (N/m2)   at T-point
   ...
   REAL(wp) , PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) :: qsr_modified, qlw_modified
   !}
   !! RD
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_blk_alloc()
      !!----------------------------------------------------------------------
      !!                ***  FUNCTION sbc_blk_alloc  ***
      !!----------------------------------------------------------------------
      ALLOCATE( qsr_modified(jpi,jpj), qlw_modified(jpi,jpj),  STAT= sbc_blk_alloc )
      !
      IF( lk_mpp            )   CALL mpp_sum ( sbc_blk_alloc )
      IF( sbc_blk_alloc > 0 )   CALL ctl_warn('sbc_blk_alloc: allocation of arrays failed')
   ...
   SUBROUTINE sbc_blk_core( kt )
      ...
      !!
! { JMM : begin modif
      REAL(wp), DIMENSION(:,:), POINTER ::   z_qsr1, z_qsr     ! temporary space for radsw corrections
      REAL(wp), DIMENSION(:,:), POINTER ::   z_qlw1, z_qlw     ! temporary space for radlw corrections
      REAL(wp), DIMENSION(:,:), POINTER ::   zxyt              ! temporary space for radlw corrections
      REAL(wp)                          ::   zzlat1, zzlat2    ! limits for ramping radiative correction
! } end modif

      CHARACTER(len=100) ::  cn_dir   !   Root directory for location of core files
      TYPE(FLD_N), DIMENSION(jpfld) ::   slf_i     ! array of namelist informations on the fields to read
      TYPE(FLD_N) ::   sn_wu, sn_wv, sn_wnd, sn_humi, sn_qsr       ! informations about the fields to be read
      TYPE(FLD_N) ::   sn_qlw , sn_tair, sn_prec, sn_snow      !   "                                 "
      TYPE(FLD_N) ::   sn_ccov                                 ! cloud cover 
      TYPE(FLD_N) ::   sn_tdif                                 !   "                                 "
      TYPE(FLD_N) ::   sn_kati, sn_katj                        ! informations for katabatic winds
      NAMELIST/namsbc_core/ cn_dir , ln_2m  , ln_taudif, rn_pfac,           &
         &                  sn_wu, sn_wv, sn_wnd, sn_humi  , sn_qsr ,           &
      ...
      IF( kt == nit000 ) THEN                   !  First call kt=nit000  !
         ...
         sn_wu   = FLD_N( 'wu10'    ,    24     ,  'wu10'    ,  .false.   , .false. ,   'yearly'  , ''       , ''         )
         sn_wv   = FLD_N( 'wv10'    ,    24     ,  'wv10'    ,  .false.   , .false. ,   'yearly'  , ''       , ''         )
         sn_wnd  = FLD_N( 'w10'     ,    24     ,  'w10'     ,  .false.   , .false. ,   'yearly'  , ''       , ''         )
         ...
         sn_ccov = FLD_N( 'ccover'  ,    24     , 'soclotot' ,  .false.   , .false. ,   'yearly'  , ''       , ''         )
         ...
         slf_i(jp_wu)   = sn_wu     ;   slf_i(jp_wv)   = sn_wv  ;  slf_i(jp_wnd) = sn_wnd
         ...
         slf_i(jp_tdif) = sn_tdif   ;   slf_i(jp_ccov) = sn_ccov
         !
         ...
         ENDIF
         IF ( sbc_blk_alloc() /= 0 )  CALL ctl_stop( 'STOP', 'sbc_blk_alloc: unable to allocate arrays' )
         !
      ENDIF
      ...
      qsr_modified(:,:) = sf(jp_qsr)%fnow(:,:,1)
      qlw_modified(:,:) = sf(jp_qlw)%fnow(:,:,1)

! { JMM : Modif of radiatives fluxes following GIG suggestion
! MJM105b : data file already have GIG correction, including high latitudes
!           missing only factor 1.1 for LW in the arctic
!         Take care to use un-modified t2 in the arctic !
!     CALL wrk_alloc( jpi,jpj, z_qsr1, z_qlw1, z_qsr, z_qlw, zxyt )

!     z_qsr1(:,:)=sf(jp_qsr)%fnow(:,:,1)
!     z_qlw1(:,:)=sf(jp_qlw)%fnow(:,:,1)

    ! correction of SW and LW in the Arctic Ocean

!     z_qlw(:,:)=1.1*z_qlw1(:,:)    ! no change

!     z_qsr(:,:)=1.0*z_qsr1(:,:)    ! -30% in the arctic
!     z_qlw(:,:)=1.0*z_qlw1(:,:)    ! no change

!     zxyt(:,:) = 0.e0
!     zzlat1 = 78.     ! ramp between zzlat1 and zzlat2
!     zzlat2 = 82.

!     WHERE ( gphit >= zzlat1 .AND. gphit <= zzlat2 )
!        zxyt = (gphit-zzlat1)/(zzlat2-zzlat1)
!     ELSEWHERE ( gphit > zzlat2 )
!        zxyt = 1.
!     ENDWHERE

!     z_qlw1(:,:)=z_qlw(:,:)*zxyt(:,:)+(1.0-zxyt(:,:))*z_qlw1(:,:)

      ! restore changed values in fnow for further use
!     qsr_modified(:,:) = z_qsr1(:,:)
!     qlw_modified(:,:) = z_qlw1(:,:)

!     CALL wrk_dealloc( jpi,jpj, z_qsr1, z_qlw1, z_qsr, z_qlw, zxyt )
!  }  End modifs
      ! in MJM95 some boundary value check on humi, prec, ccov and radiative fluxes
      !                                                        ! surface ocean fluxes computed with CLIO bulk formulea
      CALL blk_oce_core( sf, sst_m, ssu_m, ssv_m )
      ENDIF ! IF MOD( kt-1, nn_fsbc )
   ...
   SUBROUTINE blk_oce_core( sf, pst, pu, pv )
      ...
      zst(:,:) = pst(:,:) + rt0      ! converte Celsius to Kelvin (and set minimum value far above 0 K)
...
!     ENDIF
      IF( ln_dm2dc ) THEN   ;   qsr(:,:) = zztmp * sbc_dcy( qsr_modified(:,:) ) * tmask(:,:,1)
      ELSE                  ;   qsr(:,:) = zztmp *          qsr_modified(:,:)   * tmask(:,:,1)
      ENDIF
!CDIR COLLAPSE
!     zqlw(:,:) = (  sf(jp_qlw)%fnow(:,:,1) - Stef * zst(:,:)*zst(:,:)*zst(:,:)*zst(:,:)  ) * tmask(:,:,1)   ! Long  Wave
      zqlw(:,:) = (  qlw_modified(:,:) - Stef * zst(:,:)*zst(:,:)*zst(:,:)*zst(:,:)  ) * tmask(:,:,1)   ! Long  Wave
      ! ----------------------------------------------------------------------------- !
      !     II    Turbulent FLUXES                                                    !
      ! ----------------------------------------------------------------------------- !
...
! as per comment above.
      ...
      taum(:,:) = rhoa * Cd(:,:) * wndm(:,:) * wndm(:,:)
      DO jj = 1, jpj
         DO ji = 1, jpi
            zztmp = rhoa * Cd(ji,jj)
            zwnd_i(ji,jj) = zztmp * sf(jp_wu)%fnow(ji,jj,1)
            zwnd_j(ji,jj) = zztmp * sf(jp_wv)%fnow(ji,jj,1)
         END DO
      END DO
...
!CDIR NOVERRCHK
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! B grid : NO vector opt ! 
               ! ... scalar wind at I-point (fld being at T-point)  ( CASE OF  CLIMATOLOGICAL FORCING )
               zwndi_f = 0.25 * (  sf(jp_wnd)%fnow(ji-1,jj  ,1) + sf(jp_wnd)%fnow(ji  ,jj  ,1)   &
                  &              + sf(jp_wnd)%fnow(ji-1,jj-1,1) + sf(jp_wnd)%fnow(ji  ,jj-1,1)  )
               zwnorm_f = zcoef_wnorm *  zwndi_f 
               ! ... ice stress at I-point
               zwndi_f = 0.25 * (  sf(jp_wu)%fnow(ji-1,jj  ,1) + sf(jp_wu)%fnow(ji  ,jj  ,1)   &
                  &              + sf(jp_wu)%fnow(ji-1,jj-1,1) + sf(jp_wu)%fnow(ji  ,jj-1,1)  )
               zwndj_f = 0.25 * (  sf(jp_wv)%fnow(ji-1,jj  ,1) + sf(jp_wv)%fnow(ji  ,jj  ,1)   &
                  &              + sf(jp_wv)%fnow(ji-1,jj-1,1) + sf(jp_wv)%fnow(ji  ,jj-1,1)  )
               
               p_taui(ji,jj) = zcoef_wnorm * zwndi_f
               p_tauj(ji,jj) = zcoef_wnorm * zwndj_f
               ...
               z_wnds_t(ji,jj)  = sf(jp_wnd)%fnow(ji,jj,1) * tmask(ji,jj,1)
            END DO
         END DO
...
!CDIR COLLAPSE
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vect. opt.
               p_taui(ji,jj) = zcoef_wnorm2 * ( sf(jp_wu)%fnow(ji+1,jj,1) +  sf(jp_wu)%fnow(ji,jj,1))
               p_tauj(ji,jj) = zcoef_wnorm2 * ( sf(jp_wv)%fnow(ji,jj+1,1) +  sf(jp_wv)%fnow(ji,jj,1))
...
!              z_qlw(ji,jj,jl) = 0.95 * (  sf(jp_qlw)%fnow(ji,jj,1) - Stef * pst(ji,jj,jl) * zst3  ) * tmask(ji,jj,1)
               z_qlw(ji,jj,jl) = 0.95 * (  qlw_modified(ji,jj) - Stef * pst(ji,jj,jl) * zst3  ) * tmask(ji,jj,1)
               ! lw sensitivity
...
!      p_fr1(:,:) = ( 0.18 * ( 1.0 - zcoef_frca ) + 0.35 * zcoef_frca )
      p_fr1(:,:) = 0.18  * ( 1.e0 - sf(jp_ccov)%fnow(:,:,1) ) + 0.35 * sf(jp_ccov)%fnow(:,:,1)
!CDIR COLLAPSE
!     p_fr2(:,:) = ( 0.82 * ( 1.0 - zcoef_frca ) + 0.65 * zcoef_frca )
      p_fr2(:,:) = 0.82  * ( 1.e0 - sf(jp_ccov)%fnow(:,:,1) ) + 0.65 * sf(jp_ccov)%fnow(:,:,1)
